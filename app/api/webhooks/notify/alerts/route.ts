...
function authorizeNotifyWebhook(request: Request): boolean {
  ...
  if (!secret) {
    // Allow local/dev wiring until Notify confirms auth; require secret in production.
...
async function handleAlertRequest(request: Request, facilityCode?: string | null) {
